...
def get_argparser():
    ...
    parser.add_argument("--num_workers", type=int, default=0, help='number of workers (default: 1)')
    ...
    parser.add_argument(
        ...
        type=str,
        default='voc',
        choices=['voc', 'ade', 'cityscapes_domain', 'aeroscapes', 'whdld', 'marseg'],
    ...
    #CSR
    parser.add_argument("--use_csr", action="store_true", default=False)
    parser.add_argument("--weight_csr", default=0.0, type=float)
    parser.add_argument(
        "--maxstep",
        type=int,
        default=1,
        help="The max incremental step in execution (default: 1) uesd to avoid unnecessary computing in MiSS"
    )
    parser.add_argument("--csr_not_bg", action="store_true", default=False)

    #LSD
    parser.add_argument("--use_lsd", action="store_true", default=False)
    parser.add_argument("--weight_lsd", default=0.0, type=float)
    parser.add_argument("--lsd_sample_size", default=0, type=int)
    parser.add_argument("--lsd_not_bg", action="store_true", default=False)

    return parser
# loss_de loss_kd kd_mask pod pseudo num_classes test_on_val no_mask data_masking pseudo_soft
# ce_on_pseudo ce_on_new sample_weights_new classif_adaptive_factor entropy_min crop_size
# norm_act batch_size step num_workers epochs bce icarl unce nca focal_loss alpha focal_loss_gamma
# init_balanced 