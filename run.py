...
from dataset import (AdeSegmentationIncremental,
                     ...
                     whdldSegmentationIncremental,
                     transform)
from metrics import StreamSegMetrics
from segmentation_module import make_model
from train_MiSS import Trainer
...
def get_dataset(opts):
    ...
    if opts.dataset == 'voc':
        ...
        dataset = whdldSegmentationIncremental
    else:
    ...
    train_dst = dataset(
        root=opts.data_root, #--data_root data
        train=True,
...
def main(opts):
    ...
    for i, (step, lr) in enumerate(zip(copy.deepcopy(opts.step), copy.deepcopy(opts.lr))):
        print('*'*10, f'STEP {step} START', '*'*10)
        use_csr= opts.use_csr
        weight_csr = opts.weight_csr
        maxstep = opts.maxstep
        csr_not_bg = opts.csr_not_bg
        use_lsd = opts.use_lsd
        weight_lsd = opts.weight_lsd
        lsd_sample_size = opts.lsd_sample_size
        lsd_not_bg = opts.lsd_not_bg
        print('Main MiSSNet Pars:', f"{use_csr}, {weight_csr}, {maxstep}, {csr_not_bg}, {use_lsd}, {weight_lsd}, {lsd_sample_size}, {lsd_not_bg}")
        ...
        if rank == 0:
            with open(f"results/{opts.date}_{opts.dataset}_{opts.task}_{opts.name}.csv", "a+") as f:
                ...
                f.write(f"{step},{classes_iou},{val_score['Mean IoU']},*main_par:,{use_csr},{weight_csr},{maxstep},{csr_not_bg},{use_lsd},{weight_lsd},{lsd_sample_size},{lsd_not_bg}\n")

        print('*'*10, f'STEP {step} END', '*'*10)
...
def run_step(opts, world_size, rank, device):
    ...
    if TRAIN:
        ...
        for cur_epoch in range(opts.epochs):
            print(f"epoch {cur_epoch + 1} start at {time.localtime().tm_hour}:{time.localtime().tm_min}:{time.localtime().tm_sec}!!")
            # =====  Train  =====
            model.train()
